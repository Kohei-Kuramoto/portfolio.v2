  ...
  const leadership = [
    {
      ...
      sub: { en: 'Social Media Marketing · Jul 2024 – Nov 2024', ja: 'SNSマーケティング · 2024年7月〜11月' },
      bullets: {
        en: [
    ...
    },
    {
      ...
      sub: { en: 'Peer Mentor · Jun 2023 – Dec 2023', ja: 'ピアメンター · 2023年6月〜12月' },
      bullets: {
        en: [
  ...
  return (
    <section id="experience" className="py-28" style={{background: '#080C14'}}>
      <div className="max-w-5xl mx-auto px-8">
        <p style={{fontFamily: 'DM Mono, monospace', fontSize: '12px', letterSpacing: '3px', color: '#4F8EF7', textTransform: 'uppercase', marginBottom: '12px'}}>{t.experience.label[lang]}</p>
        <h2 style={{fontFamily: '"Playfair Display", serif', fontSize: 'clamp(36px, 5vw, 52px)', fontWeight: 700, color: 'white', lineHeight: 1.15, marginBottom: '16px'}}>{t.experience.title[lang]}</h2>
        <p style={{fontSize: '16px', color: '#7A8CA3', maxWidth: '560px', lineHeight: 1.7, marginBottom: '48px'}}>{t.experience.subtitle[lang]}</p>
        ...
        <div style={{padding: '32px', borderRadius: '20px', background: 'rgba(79,142,247,0.04)', border: '1px solid rgba(79,142,247,0.15)', marginBottom: '24px'}}>
          <div className="flex items-start gap-4 mb-5">
            ...
            <div>
              ...
              <p style={{fontSize: '14px', color: '#7A8CA3'}}>{lang === 'en' ? 'Data Engineer Intern · Jul 2025 – Dec 2025 · 6 months' : 'データエンジニアインターン · 2025年7月〜12月 · 6ヶ月'}</p>
            </div>
          </div>
          ...
          <ul style={{listStyle: 'none', marginBottom: '20px'}}>
            {bullets[lang].map((b, i) => (
              <li key={i} style={{fontSize: '15px', color: '#7A8CA3', padding: '6px 0 6px 20px', position: 'relative', lineHeight: 1.6}}>
          ...
          <div style={{padding: '20px', borderRadius: '12px', background: 'rgba(79,142,247,0.05)', border: '1px solid rgba(79,142,247,0.1)'}}>
            <p style={{fontSize: '11px', fontFamily: 'DM Mono, monospace', color: '#4F8EF7', letterSpacing: '1px', marginBottom: '12px'}}>{t.experience.skills[lang]}</p>
            <div className="flex flex-wrap gap-2">
              {(lang === 'en'
                ? ['Production Data Pipelines', 'ETL / ELT', 'Data Quality Monitoring', 'Cross-team Collaboration', 'Log Analysis & Debugging', 'API Integration']
                : ['本番データパイプライン', 'ETL / ELT', 'データ品質管理', 'クロスチーム連携', 'ログ解析・障害対応', 'API統合']
              ).map(s => (
        ...
        {/* Leadership */}
        <p style={{fontFamily: 'DM Mono, monospace', fontSize: '11px', letterSpacing: '3px', color: '#4F8EF7', textTransform: 'uppercase', marginBottom: '20px', marginTop: '48px'}}>🌟 {t.experience.leadership[lang]}</p>
            ...
            <div key={card.title} style={{padding: '28px', borderRadius: '16px', background: 'rgba(79,142,247,0.04)', border: '1px solid rgba(79,142,247,0.1)'}}>
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  ...
                  <p style={{fontSize: '13px', color: '#7A8CA3'}}>{card.sub[lang]}</p>
                </div>
              </div>
              <ul style={{listStyle: 'none'}}>
                {card.bullets[lang].map((b, i) => (
                  <li key={i} style={{fontSize: '14px', color: '#7A8CA3', padding: '5px 0 5px 20px', position: 'relative', lineHeight: 1.6}}>
                    <span style={{position: 'absolute', left: 0, color: '#4F8EF7', fontSize: '12px'}}>→</span>
                    {b}
                  </li>
                ))}
